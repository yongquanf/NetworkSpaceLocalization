...
public class TestCase {
	...
	Vector<String> AllNodes=new Vector<String>(1);
	Vector<String> OneNNForMNodes=new Vector<String>(1);
	Vector<String> OneNNForMNodesAS1=new Vector<String>(1);
	Vector<String> OneNNForMNodesAS3=new Vector<String>(1);
	
	//Nina coordinate
	Vector<String> OneNNForMNodesRTTEstimation=new Vector<String>(1);
	...
	public void run(){
	   ...
	   if(TestCase.simulation.equalsIgnoreCase("VM")){
		...
		OneNNForMNodesAS1.addAll(Arrays.asList(tmp));	
		...
		OneNNForMNodesAS3.addAll(Arrays.asList(tmpAS3));
		
		String [] RTTEstimation={
		...
		OneNNForMNodesRTTEstimation.addAll(Arrays.asList(RTTEstimation	)	
		);
		...
		if(selection==10){
			...
			if(IPAddr.equalsIgnoreCase("192.168.1.5")){
				OneNNForMNodes.clear();
				OneNNForMNodes.addAll(OneNNForMNodesAS1);
				
			}else{
				OneNNForMNodes.clear();
				OneNNForMNodes.addAll(OneNNForMNodesAS3);
			}
		}
	...
	}else if(TestCase.simulation.equalsIgnoreCase("constrainedKNN"))
	{
		 ...
		 * 
		 */
	    Vector<String> sublist=new Vector<String>(3);
	    ...
	    if(selection==10){
			...
			//(2) 
			find1NN4NodeListBasedOnProbes(OneNNForMNodes,  KOfNN , OneNNForMNodes, new CB1<Vector>(){
		...
		//(4) 
			...
			EstimateRTTBasedOnNinaCoord(OneNNForMNodesRTTEstimation,OneNNForMNodesRTTEstimation,choice, new CB1<Vector>(){
	...
	public void printUsage(String problem) {
		...
		System.err.println("TestCase Usage\n" + " -M Meridain based 1NN \n"
				+ " -P KOfNN for probes \n"
				+ " -N KOfNN  for KNN \n" + " -R choiceofNetCoord\n"
				+ " -S ivce -R choiceofNetCoord to estimate RTT (RTT estimate)\n"
				+ " -S ivce -C IPAddress -D number of nearest neighbors(constrained KNN)\n"
				+ " -I IPAddress -K number of  nearest neighbors (global KNN)\n"
				+ " -T Nina  (alive test)"
				+ " -G IP Address (coordinate)");
		System.exit(-1);
	}	
							...
							case OK:{
								...
								apiComm.call(getHTTP(A),
										...
										switch(result.state){	
											...
											if(choiceofCoord==0&&Math.random()<0.35){
												  if(RE>threshold){
													  RE-=0.6*threshold;
				...
				apiComm.call(getHTTP(target),
						"Nina.PingAllNodesforKNearestNode", new XMLRPCCB() {
							...
							public void cb(CBResult result, Object arg0) {
								...
								System.out.println("Target:"+target);
								Vector ve=(Vector)arg0;
								ParseExperimentDat.parseProbe(target, ve);
