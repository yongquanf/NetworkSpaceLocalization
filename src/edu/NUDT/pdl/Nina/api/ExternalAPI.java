...
import edu.NUDT.pdl.Nina.Demo_ivce.ParseExperimentDat;
...
public class ExternalAPI extends HTTPCallbackHandler implements ExternalAPIIF{
	...
	public ExternalAPI(StableManager ncManager, AbstractNNSearchManager NNManager, PingNodes _pingAllPairs) {
	 ...
	 * @param cb
	 */
	public void pingPair(String remoteNodeStr, final CB1<Double> cb){
		
		
		pingAllPairs.doPing(remoteNodeStr, new CB2<Double, Long>(){
			...
			protected void cb(CBResult result, Double arg1, Long arg2) {
				...
				case TIMEOUT:{
					...
					cb.call(result, Double.valueOf(-1));
					break;
				}
...
//															vec.add("######################");
															...
															vec.add(NetUtil.byteIPAddrToString(((NetAddress)NP.startNode).getByteIPAddr()));
...
/*		execExternalAPI.execute(  
				new Runnable() {
					...
					public void run() {		*/	
						...
						pingAllPairs.ping(addrs, new CB2<Vector<String>, Vector<Double>>(){
							@Override
							protected void cb(CBResult result, final Vector<String> arg1,
									final Vector<Double> arg2) {
								// TODO Auto-generated method stub
							
								//added by Ericfu
								//multiple thread, to speed up the main thread
								Ninaloader.execNina.execute(new Runnable(){									
									public void run() {																			
																...
																case OK: {
																	...
																	if (KNNs != null
																		...
																		while (ier.hasNext()) {
																			...
																			map.put(NetUtil.byteIPAddrToString(((NetAddress)nxtNode.startNode).getByteIPAddr()), Double.valueOf(nxtNode.rtt));
//																			addrs
...
//																						.valueOf(nxtNode.rtt));
	...
	/*	execExternalAPI.execute(  
		...
		//final Map<String, Object> map=new Hashtable<String, Object>();
		final Vector<Object> vec=new Vector<Object>(5);
		log.info("Ring: "+NNManager.MClient.instance.g_rings.CurrentNodes);
		AddressIF addr=AddressFactory.createUnresolved(TargetNode, Ninaloader.COMM_PORT);
...
//														vec.add("######################");
														...
														while (ier.hasNext()) {
															...
															vec.add(NetUtil.byteIPAddrToString(((NetAddress)nxtNode.startNode).getByteIPAddr()));
...
//				});
	...
	public void estimateRTT(final String ANode,final String BNode, final int choiceofCoord,
			final CB1<Double> cbDistance){
		
		final List<AddressIF> AllAddrs=new ArrayList<AddressIF>(10);
		int i=0;
		
			 AllAddrs.add(AddressFactory.createUnresolved(ANode, Ninaloader.COMM_PORT));
			 AllAddrs.add(AddressFactory.createUnresolved(BNode, Ninaloader.COMM_PORT));
			 ...
			 ncManager.getRemoteCoords(
					...
					new CB2<Map<AddressIF, Coordinate>, String>() {
						protected void cb(
							...
							if(addr2coord!=null&&addr2coord.size()==2){
								
								Iterator<Coordinate> ier = addr2coord.values().iterator();
								Coordinate me=ier.next();
								Coordinate you=ier.next();
												
														if(choiceofCoord==0){
														//Nina	
														double dist=me.distanceTo(you);
														Iterator<AddressIF> ier2 = addr2coord.keySet().iterator();
														AddressIF nodeA=ier2.next();
														AddressIF nodeB=ier2.next();
														double eRTT=ncManager.ifSameSubsetThenDo(nodeA,nodeB);
														if(eRTT>=0){
														...
														cbDistance.call(CBResult.OK(), Double.valueOf(dist));	
														}
															
														}else if(choiceofCoord==1){
															
													   Coordinate meV = me.VivaldiCoord;
													   Coordinate youV = you.VivaldiCoord;
														double dist=meV.distanceTo(youV);
														cbDistance.call(CBResult.OK(), Double.valueOf(dist));	
	...
	/*	execExternalAPI.execute(  
		...
		*/
		...
		int i=0;
		final List<AddressIF> AllAddrs=new ArrayList<AddressIF>(10);
		for(i=0;i<remoteNodes.size();i++){
			 AllAddrs.add(AddressFactory.createUnresolved(remoteNodes.get(i), Ninaloader.COMM_PORT));
...
*/
	...
	public void constrainedKNN(final String target, final Vector<String> peer, final int K,final CB1<Vector<Object>> cbKNN){
		...
		log.debug("XMLAPI query for constrained KNN");
			...
			NNManager.queryKNearestNeighbors(TT, candidates, K, new CB1<List<NodesPair>>(){
				...
				protected void cb(CBResult result, List<NodesPair> KNNs) {
					...
					case OK: {
						// stub
						log.info("constrained KNN branch results are returned");
						// yes, we've got KNN
						if (KNNs != null
								&& KNNs.size() > 0) {
							Iterator<NodesPair> ier = KNNs
...
//							vec.add("######################");
							...
							while (ier.hasNext()) {
								...
								ind++;
								// otherwise
								System.out.println("The "+ ind+ "th NN: "+ nxtNode.toString());
								//map.put(nxtNode.startNode.toString(), Double.valueOf(nxtNode.rtt));
...
//												.valueOf(nxtNode.rtt));
				...
				switch (result.state) {
					case OK: {
						...
						if (addrMap.get(ANode) == null) {
							...
							ncManager.estimateRTT(addrMap.get(ANode), addrMap.get(BNode), cbDistance);
						}
						break;
					}
					case TIMEOUT:
	...
	public void getRemoteCoords(final Vector<String> remoteNode,
			final CB1<Map<String, Object>> cbRemoteCoords) {
						...
						AddressFactory.createResolved(remoteNode, Ninaloader.COMM_PORT,new CB1<Map<String, AddressIF>>() {
														protected void cb(CBResult result, Map<String, AddressIF> addrMap) {
															switch (result.state) {
																case OK:
																...
																case ERROR: {
																	...
																	log.warn(result.toString());
																	cbRemoteCoords.call(result,  resultMap);;
																}
															}
														}			
													});	
	...
	public void getRemoteCoords(final Hashtable<String, Object> remoteNodesMap,
		...
		String [] nodesStr = new String[remoteNodesMap.size()];
			...
			AddressFactory.createResolved(Arrays.asList(nodesStr), Ninaloader.COMM_PORT,new CB1<Map<String, AddressIF>>() {
				protected void cb(CBResult result, Map<String, AddressIF> addrMap) {
					switch (result.state) {
						case OK:
	...
	final	Vector vec=new Vector(2);
		...
		AddressFactory.createResolved(remoteNodeStr, Ninaloader.COMM_PORT,new CB1<AddressIF>() {
			protected void cb(CBResult nsResult, AddressIF remoteNode) {
				switch (nsResult.state) {
					case OK: {
						log.debug("resolved node="+remoteNode);
					...
					case ERROR: {
						log.warn(nsResult.toString());
						Map<String, Object> resultMap = new Hashtable<String, Object>();
						resultMap.put("result", nsResult.toString());										
						cbRemoteCoord.call(nsResult, vec);
						break;
					}
